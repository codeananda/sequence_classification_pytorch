...
class UnaugmentedAnalyteDataModule(pl.LightningDataModule):
    ...
    def _scale_df_to_range(
        self, df, df_global_min=-40, df_global_max=40, scaled_min=-1, scaled_max=1
    ):
        ...
        for row in df.itertuples(index=False):
            scaled_row = self._scale_to_range(
                row,
                scaled_min,
                scaled_max,
    ...
    def _scale_seq_to_range(
        self, seq, scaled_min, scaled_max, global_min=None, global_max=None
    ):
        ...
        scaled_seq = np.array(
            [
                self._scale_one_value(
                    value, scaled_min, scaled_max, global_min, global_max
                )
                for value in seq
            ]
        )
    ...
    def _scale_one_value(self, value, scaled_min, scaled_max, global_min, global_max):
    ...
    def _reshape_to_torch_input(self, df_X, df_y, batch_first=True, y_encoding="label"):
        ...
        if y_encoding is None:
            ...
            raise ValueError(
                """Invalid value passed for y_encoding. Please
    ...
    def setup(self, stage):
        ...
        df_X_scaled, df_y = self._scale_X_y(
            df_X_unscaled,
            df_y,
        ...
        X, y = self._reshape_to_torch_input(
            df_X_scaled, df_y, batch_first=self.batch_first, y_encoding=self.y_encoding
        )
        # Split
    ...
    def train_dataloader(self):
        ...
        train_loader = DataLoader(
            dataset, self.batch_size, self.shuffle, num_workers=os.cpu_count()
        )
        return train_loader
    ...
    def val_dataloader(self):
        ...
        val_loader = DataLoader(
            dataset, self.batch_size, shuffle=False, num_workers=os.cpu_count()
        )
        return val_loader
